...
void insertArtistsHash (int* id, int count, GHashTable* newtable, int duration, char* country) {
    for (int i=0; i<count; i++) {
        Artist* artist = (Artist*) g_hash_table_lookup (hashArtist, GINT_TO_POINTER(id[i]));    // ERROR. READING MUSIC FILE INSTEAD FOR SOME REASON
...
bool checkIfAlreadyInHashTable (int id, GHashTable* newtable) {
    if (g_hash_table_lookup (newtable, GINT_TO_POINTER(id)) == NULL) return false;
    else return true;
}
